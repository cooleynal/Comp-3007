...
-- called by: case car1 of Car colour price model -> Car (unBlue colour) price model
...
hasModel :: Fleet -> String -> Bool -- takes in fleet and string and returns a boolean
hasModel Empty model = False
hasModel (AddCar (Car _ _ model0) fleet) model =
  (model0 == model)
  || (hasModel fleet model)
