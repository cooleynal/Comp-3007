...
instance Treeish Archetree where
  children  (Node _ ats) = ats

  info  (Node str _) = str

  build  str = Just . Node str
...
findSubtree p t =
  Just (:) <*> findIndex succeeds maybeChildren <*> firstSuccess maybeChildren
  where
    maybeChildren = map (findSubtree p) (children t)
...
skRuleSet =
  let x = Atom "x"
      ...
      f = Atom "f"
      g = Atom "g"
   in [ Rule {ruleLhs = App (App K x) y, ruleRhs = x},
        Rule {ruleLhs = App (App (App S f) g) x, ruleRhs = App (App f x) (App g x)}
      ]
...
assertMatch =
  let x = Atom "x"
      y = Atom "y"
   in match (App (App (App S x) y) K) (App (App (App S K) (App S K)) K)
        == Just [("x", K), ("y", App S K)]
...
applyRuleSet  sk = firstSuccess (map (\r -> applyRule r sk) skRuleSet)


assertApplyRuleSet0 =
  applyRuleSet (App (App (App S K) S) K) == Just (App (App K K) (App S K))

assertApplyRuleSet1 =
  let u = Atom "u"
      v = Atom "v"
   in applyRuleSet (App (App K (App u K)) v) == Just (App (Atom "u") K)
