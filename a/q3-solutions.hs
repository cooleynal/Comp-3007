...
instance Treeish Archetree where
  children (Node _ childs) = childs 
  info (Node information _) = information 
  build information child = Just (Node information child)
...
instance Treeish BT where
  children (BTLeaf _) = []
  children (BTNode left right) = [left,right]
  info (BTLeaf information) = information 
  info (BTNode _ _) = ""
  build information [left,right] = Just (BTNode left right)
  build _ _ = Nothing
...
instance Lensable BT where
   -- `get` traverses the tree following the path and returns the value at the end of the path
  get (Lens tree path) = go path tree
    where
      go [] (BTLeaf s) = Just (BTLeaf s)  -- If we're at a leaf, return it.
      go [] (BTNode _ _) = Nothing        -- If we reach a node without a value, return Nothing.
      go (x:xs) (BTNode left right)
        | x == 0 = go xs left   -- Go to the left child if x == 0
        | x == 1 = go xs right  -- Go to the right child if x == 1
        | otherwise = Nothing   -- Invalid path element
      go _ _ = Nothing  -- Invalid path

  -- `set` modifies the tree at the specified path and returns the modified tree
  set (Lens tree path) newVal = go path tree
    where
      go [] (BTLeaf _) = newVal   -- Replace a leaf with the new value
      go [] (BTNode _ _) = tree   -- Can't set a node directly, so we return the original tree
      go (x:xs) (BTNode left right)
        | x == 0 = BTNode (go xs left) right   -- Modify the left child if x == 0
        | x == 1 = BTNode left (go xs right)   -- Modify the right child if x == 1
        | otherwise = tree  -- Invalid path element
      go _ _ = tree  -- Invalid path, return original tree
...
findSubtree pred tree
  | pred tree = Just []  -- Found the subtree at the root
  | otherwise = search 0 (children tree)  -- Start searching in the children
  where
    -- Helper function that iterates through each child and checks for the predicate
    search _ [] = Nothing  -- No matching subtree found
    search idx (child:rest) =
      case findSubtree pred child of
        Just subpath -> Just (idx : subpath)  -- Found a match, prepend the index
        Nothing -> search (idx + 1) rest  -- Move to the next child
...
match (Atom x) sk = Just [(x, sk)]  -- Bind variable to target expression
match (App sk0 sk1) (App sk2 sk3) = do
  s1 <- match sk0 sk2  -- Match left parts
  s2 <- match sk1 sk3  -- Match right parts
  combineMatches (Just s1) (Just s2)  -- Combine substitutions
match sk0 sk1
  | sk0 == sk1 = Just []  -- Exact match without substitution needed
  | otherwise = Nothing  -- No match
...
applyRuleSet sk = applyRules skRuleSet
  where
    applyRules [] = Nothing                          -- No rule applies
    applyRules (rule:rules) =                        -- Try each rule in sequence
      case applyRule rule sk of
        Just result -> Just result                   -- Return first successful application
        Nothing -> applyRules rules                  -- Continue with remaining rules
...
reduce1 (App sk0 sk1) | succeeds (applyRuleSet sk0) = do 
  sk0' <- applyRuleSet sk0
  return (App sk0' sk1)
reduce1 (App sk0 sk1) | succeeds (applyRuleSet sk1) = do
