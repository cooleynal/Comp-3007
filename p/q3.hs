...
-- class:      header. abstract datatype that implements the function
-- instance:   source. all the implementation of the functioninstanced with specified datatype
...
instance Treeish Archetree where
  ...
  build s archtreelist = Just (Node s archtreelist) -- creating a new archtree

  -- anytime make instance of datatype, copy
...
instance Treeish BT where
  ...
  build _ [b1, b2] = Just (BTNode b1 b2)
  build _ btx = Nothing
...
-- lenseObject is BT
-- lensPath within the bt list of integers represent traversing the tree
  --

-- search until path is empty and we are at a leaf node

-- goal is to have nothign in lens object.
-- QUESTION 3
instance Lensable BT where
  get :: Lens BT -> Maybe BT
  get (Lens lo lp) = traverse lo lp
    where
      traverse (BTLeaf s) [] = Just (BTLeaf s)
      -- consider other possible pattern matches
      -- if path is not empty if 0 consider left child, if 1 consider right child
