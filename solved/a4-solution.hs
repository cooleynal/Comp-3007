...
filter p l =
  let op x z = if p x then cons x z else z
   in foldr op nil l
...
split p l =
  let op x (ts, fs) = if p x then (cons x ts, fs) else (ts, cons x fs)
   in foldr op (nil, nil) l
...
sumColumns rowLength rows =
  foldr (zipWith (+)) zeroes rows
 where
  zeroes = replicate rowLength 0
