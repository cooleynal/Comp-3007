...
eval env (List [Atom "lambda", List vars, body])
  | all isAtom vars =
      VClosure "" env (map stringit vars) body
eval env (List [Atom "lambdaf", List vars, body])
  | all isAtom vars =
      VFunc (\vals -> eval (extend (stringem vars) vals env) body)
eval env (List [Atom "let", bs, e]) =
  let (vs, es) = unzip (letBindings bs)
   in eval (extend vs (map (eval env) es) env) e
eval env (List [Atom "let*", bs, body]) =
  let envForBody = foldl (\env (x, e) -> extend [x] [eval env e] env) env (letBindings bs)
   in eval envForBody body
eval env (List [Atom "string-case", toTest, cases, catchall]) =
  case find ((== eval env toTest) . VString . fst) (stringCases cases) of
    Just (_, e) -> eval env e
    Nothing -> eval env catchall
eval env (List [Atom "letrec", List [List (Atom f : vars), fnBody], body]) =
  eval bodyEnv body
  where
    fVal vs = eval (extend (stringem vars) vs bodyEnv) fnBody
    bodyEnv = extend [f] [VFunc fVal] env
