...
difference Mt d2 = d2
difference (Entry key value point) d2 =
  let difference2 = difference point d2
  in
    if find key d2 == "" then Entry key
    (if value == "" then find key d2 else value) difference2 else difference2
...
deNeg = undefined
-- deNeg (DB rows1) =
--   all (`elem` rows1) (<0) rows1

-- all (>0) [1, 2075, 6, 1271, 1930]
